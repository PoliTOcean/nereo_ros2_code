...
from PyQt6.QtWidgets import (QMainWindow, QApplication, QLabel, QVBoxLayout, QPushButton, 
                             QTableWidget, QTableWidgetItem, QHeaderView, QDialog, QDialogButtonBox, QTextEdit, QProgressBar, QSizePolicy, QWidget)
from PyQt6.QtGui import QShortcut, QKeySequence, QPixmap, QImage, QKeyEvent
...
class ROS2ImageNode(Node):
    ...
    # To make peripheral diagnostics work we need to collect diagnostics and then count them and update the table accordingly

    def barometer_diagnostic_callback(self, msg):
        diagnostic_array = msg.status

        for i, diagnostic in enumerate(diagnostic_array):
            self.ui.peripherals_table.setItem(i, 0, QTableWidgetItem(diagnostic.name))
            self.ui.peripherals_table.setItem(i, 1, QTableWidgetItem(diagnostic.message))
            color = self.ui.control_panel.get_color(diagnostic.level)
            self.ui.peripherals_table.item(i, 1).setBackground(color)
...
class ControlPanelDialog(QDialog):
    def __init__(self):
        ...
        self.peripherals = ["Camera", "Barometer", "IMU", "Motors", "Others"]
