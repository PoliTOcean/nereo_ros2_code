...
send "cd ~/nereo_ros2_code/autostart && for f in micro_ros_connect.sh start_imu_bar.sh; do if \[ ! -f \"\$f\" \]; then echo \"ERROR: file not found: \$f\"; exit 1; fi; if \[ ! -x \"\$f\" \]; then echo \"WARNING: \$f is not executable. Fixing permissions...\"; chmod +x \"\$f\"; fi; done; echo \"Remote script check OK\"\r"
expect -re "pi@.*:.*\\$ "
...
send "tmux -f /dev/null new-session -d -s $session -n micro_ros bash -c 'cd ~/nereo_ros2_code/autostart && ./micro_ros_connect.sh; exec bash'\r"
...
send "tmux split-window -h -t $session:micro_ros bash -c 'source /opt/ros/jazzy/setup.bash && source ~/nereo_ros2_code/rpi_ws/install/setup.bash && cd ~/nereo_ros2_code/autostart && ./start_imu_bar.sh; exec bash'\r"
sleep 2
expect -re "pi@.*:.*\\$ "
...
#====================================================
#====================================================
# expect -re "pi@.*:.*\\$ "     serve? dopo tmux attach non siamo più nella shell, quindi quel promp potrebbe non arrivare mai?
#====================================================
#====================================================
