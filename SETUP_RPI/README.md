...
1. Flash the image on the SD card: using Raspberry PI Imager, flash the Ubuntu Server 22.04 LTS (64bit) image on the SD card, at least 32GB. User: `pi`, Password: `raspberry`.
...
```
Then run `sudo netplan apply`. You should then be able to shh into your PI from the ethernet connection, using `ssh politocean@10.0.0.3`. (do not forget to configure the network ip on your laptop too: you MUST set it to 10.0.0.69/24).
